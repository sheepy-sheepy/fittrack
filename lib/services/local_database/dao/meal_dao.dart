...
class MealDao {
  ...
  Future<void> deleteMeal(String mealId) async {
    await (db.delete(db.mealEntries)..where((t) => t.mealId.equals(mealId)))
        .go();
  ...
  Future<void> updateMealEntry(app_entry.MealEntry entry) async {
    await (db.update(db.mealEntries)..where((t) => t.id.equals(entry.id)))
        .write(
      MealEntriesCompanion(
        grams: Value(entry.grams),
        calories: Value(entry.calories),
  ...
  Future<void> saveWaterEntry(
      String userId, DateTime date, double amount) async {
    final existing = await (db.select(db.waterEntries)
          ..where((t) => t.userId.equals(userId) & t.date.equals(date)))
        .getSingleOrNull();

    if (existing != null) {
      await (db.update(db.waterEntries)..where((t) => t.id.equals(existing.id)))
          .write(
